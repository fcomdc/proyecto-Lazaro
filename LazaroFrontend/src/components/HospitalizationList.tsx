...
import { fetchSalas, createSala, Sala } from '../api/salas';
...
const HospitalizationList: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const [ingData, salData] = await Promise.all([fetchIngresos(), fetchSalas()]);
      setIngresos(ingData || []);
      setSalas(salData || []);
    } catch (err: any) {
  ...
  const handleCreateIngreso = async () => {
    ...
    try {
      ...
      setForm({ idPaciente: '1', idMedicoResponsable: '1', idSala: '1', motivoIngreso: '', diagnostico: '' });
      loadData();
    } catch (err: any) {
  ...
  return (
    <View style={styles.container}>
      ...
      {error ? (
        ...
        <>
          <Text style={styles.sectionHeader}>Salas Disponibles ({salas.length})</Text>
          {salas.length === 0 ? (
            ...
            <View style={styles.salasContainer}>
              {salas.map((sala, idx) => (
                <View key={sala.idSala || idx} style={styles.salaChip}>
                  ...
                  <Text style={styles.salaSub}>Capacidad: {sala.capacidad} | Estado: {sala.estado}</Text>
                </View>
              ))}
            </View>
          )}

          <Text style={styles.sectionHeader}>Ingresos Hospitalarios ({ingresos.length})</Text>
          {ingresos.length === 0 ? (
            <Text style={styles.emptyText}>No hay ingresos activos registrados.</Text>
          ) : (
            <FlatList
              data={ingresos}
              keyExtractor={(item, index) => (item.idIngreso ? item.idIngreso.toString() : index.toString())}
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.card} onPress={() => showIngresoDetail(item)}>
                  <View style={styles.rowBetween}>
                    <Text style={styles.cardTitle}>Ingreso #{item.idIngreso}</Text>
                    <Text style={styles.badge}>{item.estado}</Text>
                  </View>
                  <Text style={styles.cardText}>Fecha Ingreso: {new Date(item.fechaIngreso || '').toLocaleDateString()}</Text>
                  <Text style={styles.cardText}>Motivo: {item.motivoIngreso || 'N/A'}</Text>
                  <Text style={styles.cardText}>Diagnóstico: {item.diagnostico || 'N/A'}</Text>
                </TouchableOpacity>
              )}
            />
          )}
      ...
      <Modal visible={modalVisible} animationType="slide" transparent={true}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <ScrollView>
              <Text style={styles.modalTitle}>Registrar Ingreso Hospitalario</Text>
              ...
              <TextInput style={styles.input} value={form.idMedicoResponsable} onChangeText={t => setForm({ ...form, idMedicoResponsable: t })} keyboardType="numeric" placeholder="1" />

              <Text style={styles.label}>ID de Sala</Text>
              <TextInput style={styles.input} value={form.idSala} onChangeText={t => setForm({ ...form, idSala: t })} keyboardType="numeric" placeholder="1" />

              <Text style={styles.label}>Motivo de Ingreso</Text>
              <TextInput style={styles.input} value={form.motivoIngreso} onChangeText={t => setForm({ ...form, motivoIngreso: t })} placeholder="Cirugía / Observación" />

              <Text style={styles.label}>Diagnóstico Inicial</Text>
              <TextInput style={styles.input} value={form.diagnostico} onChangeText={t => setForm({ ...form, diagnostico: t })} placeholder="Apendicitis aguda..." />

              <View style={styles.modalButtons}>
                <TouchableOpacity style={[styles.modalBtn, styles.cancelBtn]} onPress={() => setModalVisible(false)}>
                  <Text style={styles.btnText}>Cancelar</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.modalBtn, styles.saveBtn]} onPress={handleCreateIngreso} disabled={saving}>
                  <Text style={styles.btnText}>{saving ? 'Guardando...' : 'Registrar Ingreso'}</Text>
                </TouchableOpacity>
              </View>
            </ScrollView>
          </View>
...
const styles = StyleSheet.create({
  ...
  addButton: { backgroundColor: '#17a2b8', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 6 },
  addButtonText: { color: '#fff', fontWeight: 'bold' },
  sectionHeader: { fontSize: 18, fontWeight: 'bold', marginTop: 10, marginBottom: 10, color: '#333' },
  ...
  salaChip: { backgroundColor: '#e0f7fa', padding: 10, borderRadius: 8, marginRight: 10, marginBottom: 10, borderWidth: 1, borderColor: '#b2ebf2' },
  salaTitle: { fontWeight: 'bold', color: '#006064' },
  salaSub: { fontSize: 12, color: '#00838f' },
  card: { padding: 15, borderRadius: 8, backgroundColor: '#f1f8f9', borderWidth: 1, borderColor: '#c8e6c9', marginBottom: 12 },
  rowBetween: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  cardTitle: { fontSize: 16, fontWeight: 'bold', color: '#00796b' },
  cardText: { fontSize: 14, color: '#37474f', marginTop: 2 },
  badge: { backgroundColor: '#b2dfdb', color: '#004d40', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 4, fontSize: 12, fontWeight: 'bold', overflow: 'hidden' },
